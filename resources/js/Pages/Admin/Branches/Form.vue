<template>
    <AdminLayout :title="title">
        <!-- Page Title -->
        <h2 class="text-xl text-center font-semibold leading-tight text-gray-800 dark:text-gray-200">
            {{ title }}
        </h2>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <form @submit.prevent="submit" class="p-6 space-y-6">
                        ...
                        <!-- Location Section -->
                        <section class="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Location & Delivery
                                Area</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                        ...
                        <section class="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Opening Hours & Meal
                                Times</h3>
                            <div class="space-y-6">
                                    ...
                                    <div class="flex items-start space-x-4">
                                        <div class="w-32">
                                            ...
                                            <select :id="`day-${index}`" v-model="hours.day"
                                                class="mt-1 block w-full dark:bg-gray-900 dark:text-gray-300" required>
                                                <option v-for="day in weekDays" :key="day" :value="day">{{ day }}
                                                </option>
                                            </select>
                                        </div>
                                        <div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <div class="flex gap-2">
                                                <TextInput :id="`breakfast-start-${index}`"
                                                    v-model="hours.breakfast.start" type="time"
                                                    class="mt-1 w-1/2 dark:bg-gray-900 dark:text-gray-300" />
                                                <TextInput :id="`breakfast-end-${index}`" v-model="hours.breakfast.end"
                                                    type="time"
                                                    class="mt-1 w-1/2 dark:bg-gray-900 dark:text-gray-300" />
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="flex gap-2">
                                                <TextInput :id="`lunch-start-${index}`" v-model="hours.lunch.start"
                                                    type="time"
                                                    class="mt-1 w-1/2 dark:bg-gray-900 dark:text-gray-300" />
                                                <TextInput :id="`lunch-end-${index}`" v-model="hours.lunch.end"
                                                    type="time"
                                                    class="mt-1 w-1/2 dark:bg-gray-900 dark:text-gray-300" />
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div class="flex gap-2">
                                                <TextInput :id="`dinner-start-${index}`" v-model="hours.dinner.start"
                                                    type="time"
                                                    class="mt-1 w-1/2 dark:bg-gray-900 dark:text-gray-300" />
                                                <TextInput :id="`dinner-end-${index}`" v-model="hours.dinner.end"
                                                    type="time"
                                                    class="mt-1 w-1/2 dark:bg-gray-900 dark:text-gray-300" />
                                            </div>
                                        </div>
                        ...
                        <section class="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Enable or disable this branch
                                    </p>
                                </div>
...
// Computed properties
const title = computed(() => props.branch ? 'Edit Restaurant' : 'Create Restaurant');
...
const addOpeningHours = () => {
    const availableDay = weekDays.find(day => !form.opening_hours.some(h => h.day === day)) || weekDays[0];
